...
def readDFS0(filename, scriptFolder):
	...
	clr.AddReference(
		scriptFolder +
		r"\DHI MIKE SDK\DHI.Generic.MikeZero.DFS.dll")
	clr.AddReference(
		scriptFolder +
		r"\DHI MIKE SDK\DHI.Generic.MikeZero.EUM.dll")
	...
	startdate = datetime.datetime(
		dfs0File.FileInfo.TimeAxis.StartDateTime.Year,
	...
	toMicroMeterPerSecondFactor = DHI.Generic.MikeZero.eumUtil.ConvertToBase(
		dfs0File.ItemInfo.__getitem__(0).Quantity.Unit, 1.0) * 1e6
...
def writeDFS0(gaugetime, gaugeint, outfile, scriptFolder):
	...
	MatlabDfsUtil.DfsUtil.WriteDfs0DataDouble(
		dfs, Array[float](timeMinutes), gaugeintArr)
...
def getDataPeriod(parameters, scriptFolder):
	...
	daterange = dates.num2date(gaugetime[0]).strftime(
		"%d/%m/%Y") + " - " + dates.num2date(gaugetime[-1]).strftime("%d/%m/%Y")
...
def writeLTS(parameters, scriptFolder):
	...
	try:
		...
		if strtobool(parametersDict["time_aggregate_enable"]):
			...
			dts = map(int, parametersDict["time_aggregate_periods"].split(';'))
		else:
			dts = []
